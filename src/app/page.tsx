...
const NARANJA =     { dark: '#7C3310', mid: '#C05A1A', light: '#F4CEAE', suave: '#FDF1E8' }
...
function VerticalCard({
  ...
  v: { id: string; nombre: string; descripcion: string; url: string | null; activo: boolean; icono: string }
  paleta: Paleta
}) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 12,
        border: v.activo
          ? `2px solid ${hovered ? paleta.dark : paleta.mid}`
          : `2px solid ${paleta.light}`,
    ...
    >
      <div className="flex items-start justify-between">
        <div
          style={{ background: v.activo ? paleta.dark : paleta.light, borderRadius: 10, width: 48, height: 48 }}
          className="flex items-center justify-center text-2xl flex-shrink-0"
        >
          {v.icono}
        </div>
        <span style={{
          background: v.activo ? paleta.dark : paleta.suave,
          color: v.activo ? '#fff' : paleta.mid,
      ...
      <div>
        <h3 style={{ color: v.activo ? paleta.dark : paleta.mid, fontWeight: 'bold', fontSize: 17, marginBottom: 6 }}>
          {v.nombre}
        </h3>
      ...
      {v.activo ? (
        <div style={{
          background: hovered ? paleta.dark : paleta.mid,
...
function BloqueCard({
  icono, nombre, descripcion, estado, paleta,
}: {
  icono: string; nombre: string; descripcion: string; estado: string; paleta: Paleta
}) {
  const [hovered, setHovered] = useState(false)
  return (
    <div
    ...
    >
      <div className="flex items-start justify-between">
        <div style={{ background: paleta.light, borderRadius: 10, width: 48, height: 48 }}
          className="flex items-center justify-center text-2xl flex-shrink-0">
          {icono}
        </div>
        <span style={{
      ...
      <div>
        <h3 style={{ color: paleta.dark, fontWeight: 'bold', fontSize: 17, marginBottom: 6 }}>{nombre}</h3>
...
function SectionHeader({ titulo, subtitulo, paleta, stats }: {
  titulo: string; subtitulo: string; paleta: Paleta
...
export default function Home() {
  return (
    <main className="min-h-screen" style={{ background: '#F0F4F8', fontFamily: 'Tahoma, sans-serif' }}>
      ...
      <section className="max-w-6xl mx-auto px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {verticalesSegura.map((v) => (
            <VerticalCard key={v.id} v={v} paleta={AZUL} />
          ))}
        </div>
      </section>

      {/* SEPARADOR */}
      <div style={{ height: 2, background: '#D6E4F7', maxWidth: '100%' }} className="mx-8" />
      ...
      {/* SEPARADOR */}
      <div style={{ height: 2, background: '#C8E6D0', maxWidth: '100%' }} className="mx-8" />
